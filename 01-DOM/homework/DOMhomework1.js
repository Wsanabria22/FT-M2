...
autor.innerHTML = autor.innerHTML + ' Jime!';
...
function ToDo(description) {
  // {description: 'leer', complete: false}
  // Tu código acá:
...
function buildToDo(todo, index) {
  // todo => {description: 'leer', complete: false}
  // Tu código acá:
  let toDoShell = document.createElement('div');
  toDoShell.setAttribute('class', 'toDoShell');
  toDoShell.addEventListener('click', completeToDo);
  let toDoText = document.createElement('span');
  toDoText.innerHTML = todo.description; // <div> <span id= index>leer </span> </div>
...
function buildToDos(toDos) {
  ...
  let resultado = toDos.map(function (todo, index) {
    // toDos.map(buildToDo)
    return buildToDo(todo, index); // {} => <div> <span> descripcion </span> </div>
  });
  return resultado; //[ <div> <span> descripcion </span> </div>,  <div> <span> descripcion </span> </div>,  <div> <span> descripcion </span> </div>]
}
...
function addToDo() {
  ...
  let newToDo = new ToDo(toDoValue); // {description: 'leer', complete: false}
